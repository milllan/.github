...
2. Add the required secret(s) to the repo (Settings → Secrets and variables → Actions):
   ...
   - `OPENCODE_API_KEY` if using the OpenCode Zen job (get from https://opencode.ai/auth) — forwarded as `OPENCODE_API_KEY` in the caller. Free models (e.g. `muse-spark-1.2-contributor-free`, `mimo-v2.5-free`) need no billing.
...
- Paid `muse-spark-1.2` (and `muse-spark-1.2-contributor` on the `/zen/go/v1` gateway) need a payment method (`CreditsError` 401) — not usable with a free key.
...
**Zen client-identification headers (2026-09-08):** OpenCode now requires external tools to send `x-opencode-session: <stable-id-per-conversation>` and an explicit `User-Agent` on the `/zen/go/v1` gateway (pi's `opencode-go` provider — enforced per opencode's announcement; see r/opencode "OpenCode Go is no longer general API access"). The free `/zen/v1` gateway does NOT enforce them yet — verified 2026-09-08: probes with default `curl/8.x` UA + no session header, custom UA only, session only, and both all returned byte-identical responses. The workflow sends `-A milllan-github-reviewer/1.0` and a per-run UUID session header on all zen requests preemptively. If `/zen/v1` ever starts rejecting with 403/header errors, check these first.
