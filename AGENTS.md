...
**Zen free tier is APP-ONLY (2026-09-22, standing):** all `-free` models (`mimo-v2.5-free`, `mimo-v2.6-flash-free`, `ling-3.0-flash-fin-free`, `nemotron-3-ultra-free`, `muse-spark-*-contributor-free`) return **HTTP 403 `FreeTierError`** "OpenCode's free tier can only be used from within OpenCode" from residential AND datacenter IPs, on both `/chat/completions` and `/responses` — while the same models work inside the OpenCode app/CLI (verified 2026-09-22: `opencode run -m opencode/mimo-v2.6-flash-free` answers normally). The zen review lanes are therefore disabled in this repo's caller as of 2026-09-22. The policy flapped once before (enforced ~2026-09-11, lifted by 2026-09-12), so re-probe before any re-enable:
```bash
KEY=<opencode-key> curl -sS -X POST https://opencode.ai/zen/v1/chat/completions   -A "milllan-github-reviewer/1.0" -H "x-opencode-session: $(uuidgen)"   -H "Authorization: Bearer $KEY" -H "Content-Type: application/json"   -d '{"model":"mimo-v2.6-flash-free","messages":[{"role":"user","content":"Say OK"}]}'
```
200 = re-enable the zen jobs in the caller (and wire `mimo-v2.6-flash-free` — it replaces v2.5 when available). 403 `FreeTierError` = still app-only. Paid zen models (workspace with billing) are unaffected.

**Zen free-tier caps (pre-app-only era):** the free models have tight per-model caps — `muse-spark-1.2-contributor-free` exhausted after ~10 requests in a session on 2026-09-08 (429 `FreeUsageLimitError` "Rate limit exceeded. Please try again later."). The workflow classifies that 429 type as skip-to-next-model (not retry, not fatal) so a `models` fallback list can land on a model with remaining headroom. Expect zen free reviewers to intermittently post `:warning: unavailable` comments on heavy days; that is the quota, not a bug.
