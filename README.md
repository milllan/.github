...
- **Uniform retry policy (2026-09-16):** every model gets exactly 1 retry (2 attempts, 10s backoff between) before the fallback chain skips to the next model in `models`. Removed models (400/404/410/422) and exhausted free-tier caps (429 `FreeUsageLimitError`) skip straight to the next model without retrying.
