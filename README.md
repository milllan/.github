...
- **Zen requests identify themselves**: `User-Agent: milllan-github-reviewer/1.0` + a per-run `x-opencode-session` header. Not yet enforced on the free `/zen/v1` gateway (as of 2026-09-08), but OpenCode now requires them on the `/zen/go` gateway for external tools, so the reviewer sends them preemptively.
